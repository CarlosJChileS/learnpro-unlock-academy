...
serve(async (req) => {
  ...
  try {
    ...
    const amount = prices[planType as keyof typeof prices]?.[planName as 'Basic' | 'Premium'];
    if (!amount) {
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { 
              name: `${planName} Plan (${planType})`,
              description: `LearnPro ${planName} subscription - ${planType} billing`
            },
            unit_amount: amount,
